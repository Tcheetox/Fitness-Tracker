class TwilioController < ApplicationController
    ...
    def index
        valid_user(sender_number) ? respond(message_answer) : respond("Hello dear user! Please sign in to Move&Groove and complete your profile before using our remote services.")
    end
    ...
    def message_answer 
        ...
        if s_message.first.include? "add"
            ...
            r = "Unrecognized syntax. Type 'help' to see the list of available commands."
        end
    end
    ...
    def list_handler(args) 
        if args.count == 1 && args[0] == "list"
            ...
            r = acti.count == 0 ? "You don't have any activities":
             "Previous activities: #{acti.map{|sa| "\n - #{sa.rating.humanize} #{sa.duration} min #{sa.name} on #{sa.date.strftime('%Y-%m-%d')}"}.join}"
        elsif args.count == 2 && args[0] == "list" && Activity.names.keys.include?(args[1])
            acti = @remote_user.activities.where(:name => args[1]).order("date desc").limit(10)
            r = acti.count == 0 ? "You don't have any #{args[1]} activities" : 
            "Previous #{args[1]} activities: #{acti.map{|sa| "\n - #{sa.rating.humanize} #{sa.duration} min on #{sa.date.strftime('%Y-%m-%d')}"}.join}"
        else
            r = args[0] == "list" ?  "Input error: '#{args[1]}' is not a valid activity. \nType 'help options' to see the list of activities." :
    ...
    def add_handler(args)
        ...
        if p_args.values[1] && (3..4).include?(p_args.select{|k,v| v == true}.count)
            r = try_add(args)
        else
            if !(4..5).include?(args.count) || args[0] != "add"
                r = "Input error, add an activity with this syntax: \n  add 'feeling' 'activity' 'minutes' 'km' \n Example: \n  add excellent running 50 11.20"
    ...
    def try_add(args)
        begin
            @remote_user.activities.create(:name => args[2], :date => DateTime.now(), :duration => args[3].to_i, :rating => args[1], :distance => Activity.dist(args[2])? args[4].to_f : nil)
    ...
    def help_handler(args)
        if args[1].to_s.include? "option"
            ...
            r = "List of commands:"
            r += [ "help", "help options", "add 'feeling' 'activity' 'duration' 'distance'", "list", "list 'activity'"].map{|e| "\n - #{e.humanize}"}.join
        end
        return r
    end
